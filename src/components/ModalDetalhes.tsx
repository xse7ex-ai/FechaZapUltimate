  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-3xl max-h-[92vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 bg-slate-50/50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs">
              ...
              <div className="font-bold text-slate-900 text-sm">{orcamento.clienteNome}</div>
              <div className="text-xs text-slate-600 mt-1">
                WhatsApp: <strong>{formatPhone(orcamento.clienteTelefone)}</strong>
              </div>
            </div>
            ...
            <div className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-bold text-slate-800 focus:ring-2 focus:ring-emerald-500"
                >
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-xs">
            <div className="px-4 py-3 bg-slate-100/60 border-b border-slate-200 font-bold text-xs text-slate-700 uppercase tracking-wider">
              Itens da Proposta
            </div>
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-200 space-y-1 text-xs">
              ...
              <div className="flex justify-between items-center text-sm font-extrabold text-slate-900 pt-2 border-t border-slate-200">
                ...
                <span className="text-lg text-emerald-600">{formatCurrency(orcamento.valorTotal)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            <div className="p-3 bg-white rounded-xl border border-slate-200">
              ...
              <p className="text-slate-600">{orcamento.formaPagamento || 'A combinar'}</p>
            </div>
            ...
            <div className="p-3 bg-white rounded-xl border border-slate-200">
              ...
              <p className="text-slate-600">{orcamento.prazoEntrega || 'A combinar'}</p>
            </div>
          </div>
          ...
          {orcamento.observacoes && (
            <div className="p-3 bg-white rounded-xl border border-slate-200 text-xs">
              <strong className="block text-slate-700 mb-1">Observações:</strong>
              <p className="text-slate-600 whitespace-pre-line">{orcamento.observacoes}</p>
            </div>
          )}

          {/* GEMINI AI ACTION CARD */}
          <div className="p-4 rounded-xl bg-gradient-to-r from-emerald-950 via-slate-900 to-teal-950 text-white flex flex-col sm:flex-row items-center justify-between gap-3 shadow-lg">
            <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="w-full sm:w-auto px-4 py-2 rounded-xl text-xs font-bold bg-emerald-500 hover:bg-emerald-400 text-slate-950 transition-all shadow-md shrink-0 active:scale-95"
            >
        ...
        {/* Footer Actions */}
        <div className="bg-white border-t border-slate-200 p-4 flex flex-wrap items-center justify-between gap-2.5 shrink-0">
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-300 text-slate-700 hover:bg-slate-50 text-xs font-semibold transition-colors"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-md shadow-emerald-600/25 transition-all active:scale-95"
            >
